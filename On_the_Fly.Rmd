...
## 목적
- 촬영 도중 또는 촬영 직후에 즉시 사용할 수 있는 근사 카메라 포즈를 빠르게 얻기 위함
- 초기 단계에서 GPU 친화적이고 빠른 근사 포즈 확보 후 joint optimization으로 정밀 보정

## 원칙
- **GPU friendly**
  - 메모리 접근과 연산을 GPU에서 배치로 효율적으로 처리하도록 문제를 고정 크기(fixed-size)로 구성
- **Fixed-Size Keypoint**
  - 프레임 당 많은 keypoint 대신 제한된 수(6144)를 사용해 GPU 메모리/대역폭 비용 감소
  - 유연한(가변 크기) 라이브러리(Ceres 등)를 피하고, 미니 bundle adjustment를 직접 GPU에 맞게 구현
...
- **Bootstrapping**
  ...
  - 각 3D 포인트가 고정된 수의 이미지에서 관측되도록 문제 재구성 → 고정 크기 sparse Jacobian 구성 가능
  - 블록 크기가 고정되어 memory pre-allocation과 독립적 연산 가능 → GPU에서 병렬 처리 유리
- **후속 프레임 처리**
  - 새 프레임을 최근 $N=6$ registered frames와 매칭
  - 3D-2D correspondences 위해 과거 프레임들로 triangulation하여 depth 추정 (실패 시 render된 depth 사용)
  - 이후 GPU-parallel RANSAC으로 초기 pose와 inliers 추정 및 estimator로 mini-BA로 반복 pose 정제
  - 삼각측량한 각 keypoint에 대해 3D Gaussian primitive 생성
  - 관측 및 손실(supervision)은 항상 최신 $N$개 프레임으로 제한하여 문제 크기 고정
...
## 목적
- Densification의 overhead와 단점을 피하고 각 프레임에서 즉시 적절한 **3D Gaussian primitives sampling**
- 재구성되지 않은 영역을 채우고, coarse-to-fine 방식으로 보강하여 불필요한 중복 생성을 줄여 3DGS 최적화 비용과 시간 줄임

## 원칙
- **고주파/edge 중심 배치**
  - **Laplacian of Gaussian (LoG)** 결과의 크기를 고주파/edge에 가중하여 Gaussians 배치
  - 정확한 edge 표현 위해 경계 양쪽에 2개 peak 생성
- **기존 재구성 정보 활용**
  - 현재 Active Gaussians로 rendering된 이미지의 동일한 LoG 응답을 페널티로 사용해, 이미 잘 표현된 영역에서 중복 생성 방지
- **확률 기반 sampling**
  - 픽셀별로 Gaussian 생성 확률을 정의하고, 해당 확률에 기반해 샘플링해 과밀 및 중복 제어
- **Guided matching**
