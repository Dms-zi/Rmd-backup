...
.nt-02[
1. motion model로부터 prior pose 생성 후 projection
1. coarse단계
...
.nt-02[
.pull-right.right[![:scale 70%](img/ORB/fig2.png)]
]
---
...
- 새 keyframe이 들어오면 중단됨(우선순위 낮음)
1. 과거 keyframe에서 해당 point가 보이는지 재측정후 관측 추가
1. BA에서 Tukey에 의해 가중치가 작아진(잔차가 클 경우) 원래 추적보다 좁은 radius로 해당 패치를 다시 찾음
]
...
1. Loop closing 부재
2. Occlusion 처리 부족
3. Relocalizaiton의 viewpoint 불변성 부족
4. initialization에 인간 개입 필요
---
class: inverse, middle, center, title-slide
...
???
ORB: 빠른 이진디스크림터(256bit), 회전,스케일 불변성
...
- keyframe $K_i$
    ...
    - ORB features extracted in the frame
---
### Covisibility graph
.pull-left-70.nt-01[
.f-85.nt-02[
