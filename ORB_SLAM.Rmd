...
- tracker가 제공한 pose를 새로운 keyframe pose로 저장
- reprojection해 기존 맵 포인트들을 추가로 찾아 관측 리스트에 저장
- 새로운 keyframe의 각 피라미드 레벨에서 계산해둔 FAST 코너 후보 불러옴
- 비최대 억제와 Shi-Tomasi 점수로 잘 구별되는 코너만 남김
- 기존 관측 근처인 후보 제외해 신규 3d point 후보 생성
- 각 후보를 triangulation할 두번째 뷰를 이미 있는 keyframe 중 카메라 중심과 거리가 가장 작은것 선택
- keyframe의 관찰 point의 깊의 분포로 깊이 prior 설정
...
## Feature Choice: ORB
.pull-left[
1. 이미지 피라미드 생성으로 근사적 scale invariance 확보
...
1. High speed test
    - 원 위 4개 지점 후보 먼저 검사해 적어도 3개 이상 중심 픽셀보다 밝거나 어두운지 검사
...
1. orientation 할당
    ...
    - keypoint 중심에서 centroid 방향으로의 각도로 otientation부여
    - rotation invariance
1. steered rBREIF descriptor
    - 31x31 주변 patch에서 binary test 분산이 크고 correlation이 낮은 test pair에서 픽셀쌍 선별
    - orientation에 따라 해당 pattern 회전
    - intensity비교해 256bit binary descriptor생성

]
.pull-right[![](img/ORB/orbfeature.png)]
---
