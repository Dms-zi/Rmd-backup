...
??? essential graph는 covisibility graph보다 sparse해 pose graph가빠르게최적화
---
### Covisibility graph
.f-85.nt-02[
- node=KeyFrame/ edge=두 KeyFrame이 공유하는 map point 수(가중치 θ)
- 전체 연결 정보를 담은 dense한 무향 가중 그래프.
- 로컬 맵 선택(어떤 KeyFrame들이 현재 프레임과 covisible한지), 특징 매칭 범위 결정, 키프레임 이웃 관계 판단
]

### Spanning tree 
.f-85.nt-02[
- covisibility grap를 바탕으로 시스템이 점진적으로 유지
...
### Essential Graph
.f-85.nt-02[
- node=모든 KeyFrame/ edge=Spanning tree edge + covisibility에서 가중치 큰 edgs + loop closure edge
